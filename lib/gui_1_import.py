...
def func_loaddata(self, event):
    ...
    if file_ext == '.csv':
        try:
            ...
            error_str = "The file selected is not a valid WavGliDaPro file:"
            error_str += "\n%s doesn't have column 'wg_datetime'" % fullpath
            error_str += "\n\n Try loading the raw files if all else fails."
            dlg = wx.MessageDialog(
                self, error_str, 'Import Error', wx.ICON_WARNING)
            dlg.ShowModal()
            dlg.Destroy()
            return None
...
def print_import_summary(self, file_list):
    ...
    self.TC_FileStatus.AppendText(
        'Failed:       %d (names below)\n' % num_import_errors)
    self.TC_FileStatus.AppendText(self.ImportErrors)


def print_data_summary(self):
    tmp_dates = self.data.index.to_pydatetime()
    date_sta = tmp_dates[0]
    date_end = tmp_dates[-1]
    self.TC_FileStatus.AppendText('\n')
    self.TC_FileStatus.AppendText('==============\n')
...
def update_weather_stn(self, event):
    ...
    if self.FP_weather_stn.GetPath().endswith('.txt'):
        ...
        self.TC_weather_stn.Clear()
        error_str = "The file selected is not a valid Weather station file:"
